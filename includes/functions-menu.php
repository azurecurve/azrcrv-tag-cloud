<?php
/*
	menu functions - admin menu registration, the admin page renderer, and
	the admin-post handler for saving settings.
*/
...
function add_plugin_action_link( $links, $file ) {
	...
	if ( $file === $this_plugin ) {
		$settings_link = '<a href="' . esc_url( admin_url( 'admin.php?page=' . PLUGIN_HYPHEN ) ) . '"><img src="' . esc_url( plugins_url( '../assets/images/logo.svg', __FILE__ ) ) . '" style="padding-top: 2px; margin-right: -5px; height: 16px; width: 16px;" alt="azurecurve" />' . esc_html__( 'Settings', 'azrcrv-tc' ) . '</a>';
...
function create_admin_menu() {
	...
	add_submenu_page(
		'azrcrv-plugin-menu',
		esc_html__( 'Tag Cloud Settings', 'azrcrv-tc' ),
		esc_html__( 'Tag Cloud', 'azrcrv-tc' ),
		'manage_options',
		PLUGIN_HYPHEN,
