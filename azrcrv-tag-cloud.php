	...
	if (function_exists('is_multisite') && is_multisite()){
		...
		if ($networkwide){
			...
			foreach ($blog_ids as $blog_id){
				...
				if (get_option('azrcrv-tc') === false){
					if (get_option('azc_tc_options') === false){
						add_option('azrcrv-tc', $new_options);
					}else{
						add_option('azrcrv-tc', get_option('azc_tc_options'));
					}
				}
			}
		...
		if (get_site_option('azrcrv-tc') === false){
				if (get_option('azc_tc_options') === false){
					add_option('azrcrv-tc', $new_options);
				}else{
					add_option('azrcrv-tc', get_option('azc_tc_options'));
...
function azrcrv_tc_add_plugin_action_link($links, $file){
	...
	if ($file == $this_plugin){
		$settings_link = '<a href="'.get_bloginfo('wpurl').'/wp-admin/admin.php?page=azrcrv-tc">'.esc_html__('Settings' ,'tag-cloud').'</a>';
